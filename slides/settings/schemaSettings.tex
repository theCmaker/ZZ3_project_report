\usepackage{tikz} %schémas
\usetikzlibrary{calc}
\usetikzlibrary{plotmarks}
\usetikzlibrary{scopes}
\usetikzlibrary{shapes}
\usetikzlibrary{math}
\usetikzlibrary{matrix}
\usetikzlibrary{arrows,chains,positioning,quotes}
\usetikzlibrary{snakes}
\usepackage{MnSymbol}
\usepackage{relsize}

\newcommand{\mobile}{\Large{$+$}}
\newcommand{\mobileroute}{\Large{$\oplus$}}
\newcommand{\interceptor}{\Large{$\square$}}
\newcommand{\depot}{\Large{$\square$}}

\colorlet{GoodGreen}{LimeGreen!70!Green}
\tikzset{%styles schémas
    interceptor/.style={thick,color=DarkBlue},  %intercepteur
    route/.style={interceptor},
    mobile/.style={color=LimeGreen!70!Green, circle},  %mobile
    direction/.style={mobile,dashed,thick}, %chemin mobile
    speed/.style={mobile, thick, >=latex,->}, %vitesse mobile
    uncaught/.style={color=OrangeRed},  %mobile non attrapé
    grided/.style={dotted,color=Black!20},    %grille
    schema/.style={ every node/.style=draw,
                    >=latex,
                    every join/.style=->,
                    every label/.append style={font=\scriptsize}},
    cross/.style={draw=none, red, anchor=center, font=\LARGE},
    correction/.style={red,->},
    stepnode/.style={midway,circle,solid,inner sep=1pt,gray,font=\scriptsize,fill=white},
%
    %Logigrammes
    startstop/.style={rectangle, rounded corners, minimum width=3cm, minimum height=1cm,text centered, draw=black, fill=red!40},
    io/.style={trapezium, trapezium left angle=70, trapezium right angle=110, minimum width=3cm, minimum height=1cm, text centered, draw=black, fill=blue!30},
    process/.style={rectangle, minimum width=3cm, minimum height=1.5em, text centered, text width=3cm, draw=black, fill=orange!40},
    decision/.style={diamond, minimum width=3cm, minimum height=1.5em, text centered, draw=black, fill=green!40},
    dispatch/.style={circle, minimum height=1.5em, text centered, draw=black},
    arrow/.style= {thick,-latex, rounded corners}
}
